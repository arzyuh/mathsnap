...
## Deployment (подготвено, сè уште не активно)

Проектот моментално работи само локално (два процеси - главен backend +
ракописен сервис во посебен venv, види погоре). За јавен hosting е
подготвен `Dockerfile` кој ги **обединува** двата во еден container/еден
Python environment (потврдено: EasyOCR не бара `transformers` воопшто,
конфликтот постоеше само локално поради инсталационен редослед) - без
потреба од посебен venv или HTTP повик помеѓу нив.

```bash
docker build -t mathsnap .
docker run -p 7860:7860 mathsnap
```

Опции за реален hosting (сè уште не избрани/платени):
- **Hugging Face Spaces** (Docker SDK) - веќе бара HF Pro претплата
  ($9/месец), веќе не е бесплатно за Docker/Gradio Spaces.
- **VPS** (Hetzner/DigitalOcean, ~€4.50-12/месец) - треба ≥2GB RAM
  (ракописниот модел сам зафаќа ~2.4GB тежини), nginx reverse proxy +
  HTTPS сертификат (задолжителен за камера на јавен домен, не localhost).
